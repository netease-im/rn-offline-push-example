...
public class NIMPushModule extends ReactContextBaseJavaModule implements ActivityEventListener {
    ...
    private static final String PUSH_CLICK_EVENT = "NIMPushNotificationClick";

    private static boolean isInit = false;
    private Intent lastHandledPushIntent;
    private WritableMap pendingPushClick;
    ...
    public void init(
        ...
        if(TextUtils.isEmpty(oppoAppSercet)){
            oppoAppSercet = config.optString("oppoAppSecret");
        }
    ...
    @ReactMethod
    public void getPendingPushClick(Promise promise) {
        WritableMap pushClick = pendingPushClick;
        pendingPushClick = null;
        promise.resolve(pushClick);
    }
    ...
    private void handleIntent(Intent intent, boolean emitEvent) {
        ...
        if (intent == lastHandledPushIntent) {
            return;
        }

        WritableMap pushClick = createPushClickPayload(intent);
        if (pushClick == null) {
            return;
        }

        lastHandledPushIntent = intent;
        // The React Native bridge consumes WritableMap instances when they are
        // emitted or resolved. Keep separate maps for the event and the cold-start cache.
        pendingPushClick = createPushClickPayload(intent);
        String sessionType = intent.getStringExtra("sessionType");
        String sessionId = intent.getStringExtra("sessionId");
        RNLog.i(TAG, "push click intent: sessionType = " + sessionType + " sessionId = " + sessionId);
        ...
        if (context.hasActiveCatalystInstance()) {
            context
                    ...
                    .emit(PUSH_CLICK_EVENT, pushClick);
        }
    }

    @Nullable
    private static WritableMap createPushClickPayload(Intent intent) {
        Bundle extras = intent.getExtras();
        if (extras == null || (!extras.containsKey("sessionType") && !extras.containsKey("sessionId"))) {
            return null;
        }

        WritableMap payload = Arguments.createMap();
        payload.putString("action", intent.getAction());
        payload.putString("data", intent.getDataString());

        ComponentName component = intent.getComponent();
        if (component != null) {
            payload.putString("component", component.flattenToShortString());
        }

        WritableMap extrasMap = Arguments.createMap();
        for (String key : extras.keySet()) {
            putExtra(extrasMap, key, extras.get(key));
        }
        payload.putMap("extras", extrasMap);
        return payload;
    }

    private static void putExtra(WritableMap map, String key, Object value) {
        if (value == null) {
            map.putNull(key);
